...
> Note: this page is WIP, and will be updated based on research and feedback
...
Three years on, Keystone Next is a preview of the next major release of KeystoneJS, the most
powerful headless content management system around. We've substantially rewritten the CLI, Schema
config, and Admin UI to make them more powerful and easier to use than ever before.

We've also created a new Document field type that lets you compose content from customisable
component blocks, making Keystone the first headless CMS built for the Design System generation.

#### 👇🏻 Here's what's next on our list:

## From Keystone Next to Keystone 6

When making the major changes for Keystone Next, we built around the existing stable core of
Keystone 5's packages. Now we've reached this milestone, we're focusing on completing the
transition, updating the core engine, and simplifying our architecture.

Through this process we'll continue making updates to the `@keystone-next` packages, refactoring the
core of Keystone 5 into the new engine, and laying the groundwork for the future.

There is a lot to do, and with a larger than ever team at [Thinkmill](https://www.thinkmill.com.au) working on
it, we're excited to show you what we're planning and get it shipped!

## Streamlined back-end with Prisma

[Prisma](https://www.prisma.io/) is the next-generation ORM for JavaScript and TypeScript, and we added it to Keystone
in 2020. It supports PostgreSQL, MySQL and SQLite with MS SQL and MongoDB support on the way. Today,
Prisma is mature enough for Keystone to use and is already more powerful than the Knex and Mongoose
ORMs we used in Keystone 5.
...
[discussion on GitHub](https://github.com/keystonejs/keystone-5/discussions).
