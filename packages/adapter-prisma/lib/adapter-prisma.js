const { versionGreaterOrEqualTo, flatten } = require('@keystonejs/utils');
const knex = require('knex');
const pSettle = require('p-settle');
const { BaseKeystoneAdapter, BaseListAdapter, BaseFieldAdapter } = require('@keystonejs/keystone');
const logger = require('@keystonejs/logger').logger('prisma');
...
class PrismaAdapter extends BaseKeystoneAdapter {
  ...
  async _connect({ name }) {
    console.log('GENERATE PRISMA SCHEMA', name);
    const lines = [
      'datasource db {',
      '  provider = "sqlite"',
    ...
    const models = Object.values(this.listAdapters).map(listAdapter => listAdapter._createModel());
    console.log(models);
    lines.push(...flatten(models));
    console.log('---');
  ...
  getDefaultPrimaryKeyConfig() {
    ...
    console.log(this.name, AutoIncrement.primaryKeyDefaults);
...
class PrismaListAdapter extends BaseListAdapter {
  ...
  async _create(data) {
    console.log({ data });
    const result = await this.parentAdapter.prisma.todo.create({ data });
    console.log({ result });
    return result;
  ...
  async _itemsQuery(args, { meta = false, from = {} } = {}) {
    ...
    console.log({ args, meta, from });
    console.log(this.parentAdapter.prisma[this.key.toLowerCase()]);
    console.log(await this.parentAdapter.prisma[this.key.toLowerCase()].findMany());
    return await this.parentAdapter.prisma[this.key.toLowerCase()].findMany();
  }
}
