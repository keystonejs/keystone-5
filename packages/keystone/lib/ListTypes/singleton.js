...
module.exports = class List {
  constructor(
    ...
    { getListByKey, adapter, defaultAccess, registerType, isAuxList, createAuxList, schemaNames }
  ) {
    this.key = key;
    this._fields = fields;
    this._hooks = hooks;
    ...
    const _singular = _label;
    const _plural = _label;
  ...
  async _initSingletonItem() {
    let item = await this.adapter.itemsQuery({ first: 1 })[0];
    if (!item) {
      item = await this.adapter.create({});
    }
    return item.id;
  }
  ...
  getAdminMeta({ schemaName }) {
    ...
    return {
      ...
      singular: this.adminUILabels.label,
      plural: this.adminUILabels.label,
  ...
  async itemQuery(context, gqlName, info) {
    const id = await this._initSingletonItem();
    const operation = 'read';
    graphqlLogger.debug({ operation, type: opToType[operation], gqlName }, 'Start query');
    ...
    graphqlLogger.debug({ operation, type: opToType[operation], gqlName }, 'End query');
    return result;
  }
  ...
  async updateMutation(data, context, mutationState) {
    const operation = 'update';
    const gqlName = this.gqlNames.updateMutationName;

    // ToDo: Thds is a little lazy, I should probably change _updateSingle
    // Adding ID here for now
    const id = await this._initSingletonItem();
  ...
  _wrapFieldResolver(field, innerResolver) {
    return async (item, args, context, info) => {
      // Check access
      const operation = 'read';
      // How does this work with list??? Looks like it has the wrong signature
      const access = await context.getFieldAccessControlForUser(
        field.access,
        this.key,
        field.path,
    ...
    if (schemaAccess.read) {
      resolvers = {
        ...
        [this.gqlNames.itemQueryName]: (_, args, context, info) => {
          return this.itemQuery(context, this.gqlNames.itemQueryName, info);
        },
      };
    }
  ...
  gqlMutationResolvers({ schemaName }) {
    ...
    const updateFields = this.getFieldsWithAccess({ schemaName, access: 'update' });
    if (schemaAccess.update && updateFields.length) {
      mutationResolvers[this.gqlNames.updateMutationName] = (_, { data }, context) =>
        this.updateMutation(data, context);
    }
